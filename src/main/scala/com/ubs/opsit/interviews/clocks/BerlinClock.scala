...
object BerlinClock {
  /**
   * the Function generates BerlinClock specific instance of StringParsingClock
   * Time String will be processed by java.time.DateTimeFormatter.ISO_LOCAL_TIME with ResolverStyle.STRICT though
   * exceptions will be thrown accordingly
   * @tparam P the type of the value generated by ClockRenderer implicitly or explicitly provided to the BerlinClock
   *           to be created during time conversion
   * @return BerlinClock specific instance of StringParsingClock
   */
  ...
  implicit def BerlinClockConverter[P: ClockRenderer] = new StringParsingClock[BerlinClock[P]] {
    def fromString(aTime: String): BerlinClock[P] = aTime match {
      /**
       * As soon as 24:00:00 value of time (i.e. end of the day) does not supported by standard Java libs the one shoud be
       * provided as custom value case
       */
   ...
   * @tparam P the type of the value generated by ClockRenderer implicitly or explicitly provided to the BerlinClock
   *           to be created during time conversion
   * @return instance of the BerlinClock set to the 24:00:00
   */
